...
//A slot is generated for every tile by default. Custom means generating a list of binary values for each slot in your grid, if the slot count exceeds your custom count, all remaining slots will not be generated
slot_position = "All"; //["All", "Staggered", "Edge Rows", "Edge Columns", "Corners","Custom"]
...
// Create position matrix for excluded slots from input string
function correct_input_length(string, l) = len(string) > l 
  ? substr(string,0,l)
  : str_pad(string,l,"0");
function create_pos_matrix(string, rows) = list_to_matrix(string, rows, "0");

// Helper function to locate excluded positions from input matrix
function find_zero_pos(mat) = 
  [ for (i = [0 : len(mat) - 1])
      for (j = [0 : len(mat[i]) - 1])
        if (mat[i][j] == "0") [i,j]
  ];

function get_except_pos(string, hgrids, vgrids) = let ( l = hgrids * vgrids) find_zero_pos(create_pos_matrix(correct_input_length(string, l), vgrids));
...
function is_grid_distribute(hgrid, vgrid, max_hgrid, max_vgrid, distri, except_pos = []) =
  ...
  ) in_list([hgrid, vgrid], except_pos) ? false : (distri == "All") || (distri == "Staggered" && is_stagger) || (distri == "Corners" && is_corner) || (distri == "Top Corners" && is_top_corner) || (distri == "Bottom Corners" && is_top_corner) || (distri == "Edge Rows" && is_edge_row) || (distri == "Edge Columns" && is_edge_column) || (distri == "Custom");
...
// If custom positioning is chosen, calculate the except_pos variable, otherwise do nothing

except_slot_pos = slot_position == "Custom" ? get_except_pos(except_slot_string, plate_horizontal_size, plate_vertical_size) : [];
echo(str(except_slot_pos));
//if (slot_position == "Custom") except_pos = get_except_pos(except_slot_string, plate_horizontal_size, plate_vertical_size);
  //echo("nothing here");
...
module main_generate() {
  ...
  if (slot_type != "None") {
    ...
    // Insert segment dealing with excluded slots here
